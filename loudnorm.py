...
def main():
    ...
    parser.add_argument(
        "--dataroot", type=str, required=True, help="Location of the files"
    )
    parser.add_argument(
        "--output", type=str, required=True, help="loaction of londnorm audio"
    )
    parser.add_argument(
        "--fixed_loudness", type=float, default=-23.0, help="target loudness"
    )
    ...
    for f in files:
        ...
        # reference paper: https://www.itu.int/dms_pubrec/itu-r/rec/bs/R-REC-BS.1770-4-201510-I!!PDF-E.pdf
        meter = pyln.Meter(sr, block_size=0.200)

        loudness = meter.integrated_loudness(data)
        data_normalized = pyln.normalize.loudness(
            data, loudness, target_loudness=opt.fixed_loudness
        )
