...
from data.image_folder import make_scp_dataset
from data.image_folder import make_scp_test_dataset
...
class UnpairedscpDataset(BaseDataset):
    """

    This dataset class can load unaligned/unpaired scp format datasets
    it requires two directories to host train data,
    it called domain A (here, I assume it is clean speech data) and domain B
    (here, I assume it is target noisy speech data)

    it also requires two directories to host valid data,
    it called domain A (here, I assume it is clean speech data) and domain B
    (here, I assume it is target noisy speech data)
    ...
    def __init__(self, opt):
        ...
        if opt.state == "Train":
            self.A_paths = make_scp_dataset(opt.dir_trainA, opt.max_dataset_size)
            self.B_paths = make_scp_dataset(opt.dir_trainB, opt.max_dataset_size)

        if opt.state == "Valid":
            self.A_paths = make_scp_dataset(opt.dir_validA, opt.max_dataset_size)
            # 2021-11-16 modified, change self.A_paths into self.B_paths
            #self.A_paths = make_scp_dataset(opt.dir_validB, opt.max_dataset_size)
            self.B_paths = make_scp_dataset(opt.dir_validB, opt.max_dataset_size)
        if opt.state == "test":
            self.A_paths = make_scp_test_dataset(opt.dir_testA, opt.max_dataset_size)
        ...
        # Compute the spectrogram components parallelly to make it more efficient;
        # uses Joblib to maintain input data
        self.clean_specs = Parallel(n_jobs=self.num_cores, prefer="threads")(
            delayed(processInput)(i, self.spec_power, self.state, self.channels)
        ...
        # Having an OrderedDict to access no. of components,
        # so we can wait before generation to collect all components
        ...
        if self.state == "Train":
            ##Preparing domainB dataset is only required if we are in the Training state
            # for generation,  we don't require domainB
